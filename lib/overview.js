...
module.exports = function(appConfig) {
	...
	views = fs.readdirSync(cfg.sources.views);
	layouts = fs.readdirSync(cfg.sources.layouts);
	moduleCandidates = fs.readdirSync(modulePath);
	pinnedViews = require(path.join(cfg.appPath, cfg.configPath, 'pinned-views.json'));
...
function getModuleTemplates(dir, modName) {
	return fs.readdirSync(path.join(cfg.sources.modulesBaseDir, dir)).filter(isHbsFile).map(function(file) {
		return file2Template(file, dir, modName);
	});
}

function repositoryUri(dir) {
	dir = path.relative(cfg.appPath, dir);
	return cfg.repository ? (cfg.repository + dir) : null;
}

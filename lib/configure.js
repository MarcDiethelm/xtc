...
var nodeEnv     = process.env.NODE_ENV !== 'production' ? 'development' : 'production'
   ...
   ,fs          = require('fs')
   ,url         = require('url')
   ,appPath     = process.env.PWD // where the process was started. Project's package.json containing configPath is there.
   ,xtcPath     = require.resolve('./configure').split(path.sep +'lib')[0]
   ,pkg         = require(path.join(process.env.PWD, 'package.json'))
   ,xtcPkg      = require(path.join(appPath, 'package.json'))
   ...
   ,projectConfigPath = path.resolve(process.env.PWD, pkg.configPath)
...
var configurePaths = function configurePaths(cfg) {
	...
	cfg.set('routesPath',    path.resolve(xtcPath, cfg.get('routesPath')));

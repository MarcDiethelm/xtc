...
var nodeEnv     = process.env.NODE_ENV !== 'production' ? 'development' : 'production'
   ...
   ,path        = require('path')
   ,fs          = require('fs')
   ,appPath     = require.resolve('./configure').split('/lib')[0]
   //,appPath     = path.dirname(require.resolve('./configure'))
   ...
   ,projectConfigNames = require( path.join(appPath, projectConfigPath, 'configs.json') )
;
...
var configurePaths = function configurePaths(cfg) {
	var sourcesBasePath = cfg.get('sourcesBasePath')
	   ,sources = cfg.get('sources')
	   ,build = cfg.get('build')
	   ,buildBaseUri
	   ,buildDir = nodeEnv == 'development' ? build.baseDirNameDev : build.baseDirNameDist
	   ,buildPath = path.join(cfg.get('buildBasePath'), buildDir)
	   ,keys, key
	   ,i
	;
	...
	cfg.set('routesPath',    path.resolve(cfg.get('routesPath')));
	cfg.set('buildBasePath', path.resolve(cfg.get('buildBasePath')));
...
function makeBuildPaths(cfg) {
	var build = cfg.get('build')
	   ,buildDir = nodeEnv == 'development' ? build.baseDirNameDev : build.baseDirNameDist
	   ,buildPath = path.join(cfg.get('buildBasePath'), buildDir)
	;

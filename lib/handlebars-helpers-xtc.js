...
helpers.inline = function(inlineAssetName, options) {
	var  baseDirName = NODE_ENV == 'development' ? 'baseDirNameDev' : 'baseDirNameDist'
		,file = path.join(cfg.pathsAbsolute.staticBaseDir, cfg.static.build[baseDirName], cfg.static.build[inlineAssetName].inline[NODE_ENV])
...
helpers.test = function() {
	...
	var tmp = testTemplate({
		...
		,'static': {
			...
			,build: {test: cfg.staticUriPrefix + '/' + cfg.static.build.baseDirNameDev +'/test.js'}
		}

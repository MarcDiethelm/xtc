...
function getModTemplate(options) {
	var  moduleName = options.name
		...
		,file = path.join(modDir, template + cfg.templateExtension)
		,returnValue
	;
	...
	try {
		...
		err = utils.error('Can\'t read template file. Module: ' + moduleName + ', Template: ' + templateName + cfg.templateExtension, e);
	...
	if (annotateModules) {
		...
		returnValue.file = template + cfg.templateExtension;

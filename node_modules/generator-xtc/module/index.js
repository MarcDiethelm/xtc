	...
	if (!this.customize || this.needHtml) {
		if (this.xtcCfg.customModuleTemplatePath) {
			...
			if (stat.isFile()) {
				...
				this.copy(path.resolve(this.projectPath, this.xtcCfg.customModuleTemplatePath), path.join(this.modulesDir, this.name + (this.xtcCfg.templateExtension || '.hbs' )));
			}
			else if (stat.isDirectory()) {
				this.directory(path.resolve(this.projectPath, this.xtcCfg.customModuleTemplatePath), path.join(this.modulesDir));
			}
			else {
				log(chalk.red('The custom module template can\'t be found: %s', this.xtcCfg.customModuleTemplatePath));
				process.exit(1);
			}
		}
		else {
			...
			this.mkdir(this.modulesDir);
			this.template('_module.hbs', path.join(this.modulesDir, this.name + (this.xtcCfg.templateExtension || '.hbs' )));
		}
	}
	(!this.customize || this.needCss) && this.template('_module.less', path.join(this.modulesDir, this.name +'.less'));

	if (!this.customize || this.needJs) {
		this.template('_module.js', path.join(this.modulesDir, this.name +'.js'));
